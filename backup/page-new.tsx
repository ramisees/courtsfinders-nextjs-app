...
import { courtSearchService } from '@/lib/court-search-service';
...
interface Court {
  ...
  photos?: Array<{ photoReference: string; height: number; width: number }>;
}
...
export default function Home() {
  ...
  const handleSearch = async (location: string, sport: string) => {
    setIsLoading(true);
    try {
      const results = await courtSearchService.searchCourts(location, sport);
      setCourts(results);
    } catch (error) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleSearch(searchQuery, selectedSport)}
                      disabled={!searchQuery.trim() || isLoading}
                      className="flex-1 bg-primary-500 hover:bg-primary-600 disabled:bg-gray-600 disabled:cursor-not-allowed text-white px-6 py-3 rounded-lg font-semibold transition-colors duration-200 flex items-center justify-center"
                    >
                      {isLoading ? (
                    ...
                    <FindNearMe 
                      ...
                      onSearch={(location, sport) => handleSearch(location, sport)}
      ...
      {courts.length > 0 && (
        <section className="py-16 px-4 sm:px-6 lg:px-8 bg-dark-800">
          <div className="max-w-7xl mx-auto">
            ...
            <SearchResults courts={courts} isLoading={isLoading} />
          </div>
        </section>
      )}
